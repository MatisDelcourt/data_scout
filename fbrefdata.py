...
ScoutReport(season='21-22',  # Season (In the future I'll upload code on my GitHub that will download prior years' data)
            program='fw',  # Program to run
            player_pos='Centre-Forward',  # See options below
            playerPrompt='Tammy Abraham',  # Player name
            ...
            compP='serie a',  # OPTIONS: epl, la liga, bundesliga, serie a, ligue 1 ('n' will be all 5 leagues)
            saveinput='y',  # 'y' if you want to save figure, 'n' if not
            signature='@Handle'  # Your twitter or website handle
            )
...
def ScoutReport(season, program, player_pos, playerPrompt, SquadPrompt, minutesPlayed, compP, saveinput, signature):
    ...
    def midfielders():
        ...
        plt.subplots_adjust(hspace=1)
        plt.suptitle(
            '%s (%i, %s, %s) - All values except percentages are per 90 minutes. pAdj = Possession-Adjusted\n Compared to %s %ss, %i+ minutes | Data from StatsBomb(FBRef) | %s | Code by @BeGriffis'
            % (playerName, int(age), teamName, ssn, Comp, player_pos, minutesPlayed, signature),
            fontsize=16,
            color="#4A2E19", fontweight="bold", fontname="DejaVu Sans")
        plt.style.use("default")
    ...
    def goalkeepers():
        ...
        plt.subplots_adjust(hspace=1)
        plt.suptitle(
            '%s (%i, %s, %s) - All values except percentages & distance are per 90 minutes.\n Compared to %s %ss, %i+ minutes | Data from StatsBomb(FBRef) | %s | Code by @BeGriffis'
            % (playerName, int(age), teamName, ssn, Comp, player_pos, minutesPlayed, signature),
            fontsize=18,
            color="#4A2E19", fontweight="bold", fontname="DejaVu Sans")
        plt.style.use("default")
