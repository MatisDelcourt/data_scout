...
for idx, data in enumerate(data_to_load):
    ...
    # Determine file path
    file_path = f"C:/Users/Matis/Documents/Sorare_Data/Graph_Building/data_directory/whoscored_data/{data[2]}_{str(int(data[2].replace('20', '')) + 1)}/{data[1]}"
...
for idx, dataset in all_data.iterrows():
    ...
    inplay_touches = touches[
        touches['satisfiedEventsTypes'].apply(lambda x: False if (5 in x or 31 in x or 34 in x or 212 in x) else True)]
    suc_touch_opp3 = inplay_touches[
        (inplay_touches['outcomeType'] == 'Successful') & (inplay_touches['x'] >= 100 * (2 / 3))]
    suc_touch_box = inplay_touches[
        (inplay_touches['outcomeType'] == 'Successful') & (inplay_touches['x'] >= 83) & (inplay_touches['y'] <= 79) & (
                    inplay_touches['y'] >= 21)]
    ...
    pass_opp3 = player_events[(player_events['eventType'] == 'Pass') & (player_events['x'] < 100 * (2 / 3)) & (
                player_events['endX'] >= 100 * (2 / 3))]
    ...
    suc_carry_opp3 = player_events[(player_events['eventType'] == 'Carry') & (player_events['x'] < 100 * (2 / 3)) & (
                player_events['endX'] >= 100 * (2 / 3))]
    ...
    player_def_hull = wce.create_convex_hull(player_def_actions, name=dataset['player'], min_events=5,
                                             include_events='1std', pitch_area=10000)
    player_def_hull = wce.passes_into_hull(player_def_hull.squeeze(),
                                           player_match_events[player_match_events['teamId'] != team_id], xt_info=True)
    ...
    playerinfo = wde.group_player_events(suc_touch_opp3, playerinfo, group_type='count',
                                         primary_event_name='suc_touch_opp3')
    playerinfo = wde.group_player_events(suc_touch_box, playerinfo, group_type='count',
                                         primary_event_name='suc_touch_box')

    playerinfo = wde.group_player_events(suc_prog_pass, playerinfo, group_type='count',
                                         primary_event_name='suc_prog_pass')
    playerinfo = wde.group_player_events(suc_prog_pass_opph, playerinfo, group_type='count',
    ...
    playerinfo = wde.group_player_events(suc_pass_opp3, playerinfo, group_type='count',
                                         primary_event_name='suc_pass_opp3')
    playerinfo = wde.group_player_events(suc_pass_box, playerinfo, group_type='count',
                                         primary_event_name='suc_pass_box')
    playerinfo = wde.group_player_events(suc_pass, playerinfo, group_type='sum', event_types=['xThreat'])

    playerinfo = wde.group_player_events(suc_prog_carry, playerinfo, group_type='count',
                                         primary_event_name='suc_prog_carry')
    playerinfo = wde.group_player_events(suc_prog_carry_opph, playerinfo, group_type='count',
                                         primary_event_name='suc_prog_carry_opph')
    playerinfo = wde.group_player_events(suc_carry_opp3, playerinfo, group_type='count',
                                         primary_event_name='suc_carry_opp3')
    playerinfo = wde.group_player_events(suc_carry_box, playerinfo, group_type='count',
    ...
    playerinfo = wde.group_player_events(interceptions, playerinfo, group_type='count',
...
pass_opp3 = comp_events[
    (comp_events['eventType'] == 'Pass') & (comp_events['x'] < 100 * (2 / 3)) & (comp_events['endX'] >= 100 * (2 / 3))]
suc_pass_opp3 = pass_opp3[pass_opp3['outcomeType'] == 'Successful']
...
suc_carry_opp3 = comp_events[
    (comp_events['eventType'] == 'Carry') & (comp_events['x'] < 100 * (2 / 3)) & (comp_events['endX'] >= 100 * (2 / 3))]
...
for idx, player_stat in player_stats.iterrows():
    ...
    for stat in ['suc_touch_box_90', 'suc_touch_opp3_90', 'suc_action_opp3_90', 'suc_prog_pass_90', 'suc_prog_carry_90',
                 'xt_pass_90', 'xt_carry_90', 'suc_tackles_90', 'dribble_prevented_pct', 'interceptions_90',
                 'aerials_won_90', 'aerial_win_pct', 'cross_complete_pct', 'recoveries_90', 'suc_prog_action_90',
                 'suc_action_box_90', 'suc_cross_90']:
        ascending = False
        rank_str = stat + '_rank'
...
gs = fig.add_gridspec(2, 4, left=0.21, right=0.98, bottom=0.1, top=0.94, wspace=0.05, hspace=0.2,
                      width_ratios=[1, 1, 1, 2], height_ratios=[2, 2])
...
fig.text(0.1, text_start_y - 0.03, player_stats.loc[0, 'position'] + ", " + player_stats.loc[0, 'team'], fontsize=12,
         fontweight="regular", color='w', ha="center")
fig.text(0.1, text_start_y - 0.057,
         f"{player_stats.loc[0, 'league'].replace('_', ' ')}, {player_stats.loc[0, 'year']}/{int(player_stats.loc[0, 'year']) + 1}",
         fontsize=9, fontweight="regular", color='w', ha="center")
...
fig.text(0.188, stat_start_y, int(player_stats.loc[0, 'matches']), fontsize=11, fontweight="regular", color='w',
         va="center", ha="right")

fig.text(0.012, stat_start_y - 0.05, "Tackles:", fontsize=11, fontweight="bold", color='w', va="center")
fig.text(0.188, stat_start_y - 0.05, int(player_stats.loc[0, 'suc_tackles']), fontsize=11, fontweight="regular",
         color='w', va="center", ha="right")
...
bar1.axis("off")
fig.text(0.188, stat_start_y - 0.095,
         f"Rank: {int(player_stats.loc[0, 'suc_tackles_90_rank'])}/{int(player_stats.loc[0, 'comparison_pts'])}",
         fontsize=6, color='w', ha="right", va="center")

fig.text(0.012, stat_start_y - 0.125, "Tackle Success:", fontsize=11, fontweight="bold", color='w', va="center")
fig.text(0.188, stat_start_y - 0.125, str(round(player_stats.loc[0, 'dribble_prevented_pct'], 2)) + "%", fontsize=11,
         fontweight="regular", color='w', va="center", ha="right")
...
bar2.axis("off")
fig.text(0.188, stat_start_y - 0.17,
...
fig.text(0.188, stat_start_y - 0.2, int(player_stats.loc[0, 'interceptions']), fontsize=11, fontweight="regular",
         color='w', va="center", ha="right")
...
bar2.axis("off")
fig.text(0.188, stat_start_y - 0.245,
...
fig.text(0.188, stat_start_y - 0.275, int(player_stats.loc[0, 'recoveries']), fontsize=11, fontweight="regular",
         color='w', va="center", ha="right")
...
bar2.axis("off")
fig.text(0.188, stat_start_y - 0.32,
...
fig.text(0.188, stat_start_y - 0.35, str(round(player_stats.loc[0, 'aerial_win_pct'], 1)) + "%", fontsize=11,
         fontweight="regular", color='w', va="center", ha="right")
...
bar2.axis("off")
fig.text(0.188, stat_start_y - 0.395,
...
fig.text(0.188, stat_start_y - 0.425, str(round(player_stats.loc[0, 'cross_complete_pct'], 1)) + "%", fontsize=11,
         fontweight="regular", color='w', va="center", ha="right")
...
bar2.axis("off")
fig.text(0.188, stat_start_y - 0.47,
...
fig.text(0.188, stat_start_y - 0.5, int(player_stats.loc[0, 'suc_touch_opp3']), fontsize=11, fontweight="regular",
         color='w', va="center", ha="right")
...
# Heatmap
full_pitch.kdeplot(player_inplay_touches['x'], player_inplay_touches['y'], fill=True, levels=80, shade_lowest=True,
                   cmap='viridis', cut=8, alpha=1, antialiased=True, zorder=0, ax=pitch1)
full_pitch.kdeplot(player_inplay_touches['x'], player_inplay_touches['y'], fill=True, levels=100, shade_lowest=True,
                   cmap='viridis', cut=8, alpha=1, antialiased=True, zorder=0, ax=pitch1)
...
pitch2.scatter(player_hull['hull_centre'][1], player_hull['hull_centre'][0], marker='H', color='cornflowerblue',
               alpha=0.6, s=300, zorder=4)
pitch2.scatter(player_hull['hull_centre'][1], player_hull['hull_centre'][0], marker='H', edgecolor='cornflowerblue',
               facecolor='none', alpha=1, lw=2, s=300, zorder=4)
pitch2.text(player_hull['hull_centre'][1], player_hull['hull_centre'][0], player_initials, fontsize=7,
            fontweight='bold', va='center', ha='center', color='w', zorder=7, path_effects=path_eff)
...
for single_pass in player_hull['suc_pass_into_hull']:
    ...
    if pass_xt < 0.01:
        ...
        line_alpha = 0.2
        zorder = 6
    full_pitch.lines(single_pass[0][0], single_pass[0][1], single_pass[1][0], single_pass[1][1], lw=2, comet=True,
                     capstyle='round',
...
for _, single_pass in player_suc_prog_passes.iterrows():
    ...
    if pass_xt < 0.005:
        ...
        line_alpha = 0.2
        zorder = 2
    full_pitch.lines(single_pass['x'], single_pass['y'], single_pass['endX'], single_pass['endY'], lw=2, comet=True,
                     capstyle='round',
...
for _, single_carry in player_suc_prog_carries.iterrows():
    ...
    if pass_xt < 0.005:
        ...
        line_alpha = 0.2
        zorder = 2
    full_pitch.lines(single_carry['x'], single_carry['y'], single_carry['endX'], single_carry['endY'], lw=2, comet=True,
                     capstyle='round',
...
bin_statistic_2 = full_pitch.bin_statistic(player_suc_passes['x'], player_suc_passes['y'],
                                           statistic='sum', bins=(6, 5), normalize=True,
                                           values=player_suc_passes['xThreat_gen'])
...
stats_to_tabulate = ['suc_tackles_90', 'dribble_prevented_pct', 'interceptions_90', 'aerials_won_90', 'aerial_win_pct',
                     'recoveries_90',
                     'suc_touch_box_90', 'suc_prog_action_90', 'suc_action_opp3_90', 'suc_action_box_90', 'xt_pass_90',
                     'xt_carry_90',
                     'suc_cross_90', 'cross_complete_pct']
stat_names = ['Tackles / 90', 'Tackle Success %', 'Interceptions / 90', 'Aerial Duels Won / 90',
              'Aerial Duel Success %', 'Recoveries / 90',
              'Opp. Box Touches / 90', 'Progressive Actions / 90', 'Actions into Opp 3rd / 90',
...
for idx in np.arange(0, num_cols + 1):
    ...
    if idx != num_cols:
        ...
        tab_ax.text(xpos_txt, (1 + header_start_height) / 2,
                    f"{player_stats.loc[idx, 'name'].split(' ')[0][0]} {player_stats.loc[idx, 'name'].split(' ')[1]}\n{int(player_stats.loc[idx, 'year'].replace('20', ''))}/{1 + int(player_stats.loc[idx, 'year'].replace('20', ''))}",
...
for idx in np.arange(0, num_rows + 1):
    ...
    if idx != num_rows:
        ...
        for idx_2 in np.arange(0, num_cols):
            ...
            tab_ax.text(xpos_txt, ypos_txt, round(player_stats.loc[idx_2, stats_to_tabulate[idx]], 2), fontsize=8,
                        color='w', va="center")
            tab_ax.scatter(xpos_rnk, ypos_txt, s=3500 * row_height, marker='s', color=table_cmap(int(255 * (
                        player_stats.loc[idx_2, stats_to_tabulate[idx] + "_rank"] / player_stats.loc[
                    idx_2, 'comparison_pts']))))
            tab_ax.text(xpos_rnk, ypos_txt, int(player_stats.loc[idx_2, stats_to_tabulate[idx] + "_rank"]), fontsize=8,
                        fontweight='bold', va='center', ha='center', color='w', path_effects=path_eff)
